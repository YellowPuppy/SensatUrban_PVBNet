...
at::Tensor hash_query_cuda(const at::Tensor hash_query,
  ...
  const int nextPow2 = pow(2, ceil(log2((double)n))); //涵盖n的最小平方数
  ...
  // bucket overflow.
  /**
     当 n 很大时，散列值往往分布更均匀，通常选择 table_size 为 2 * nextPow2 就足够了。
     对于较小的n，散列值分布不均的影响更加明显，因此我们选择table_size为4 * nextPow2以减少桶溢出的机会。
  */
