...
from core.utils.helper_metrics import batch_mIoU, mIoU
...
class SensatUrbanEDA(object):
    ...
    def grid_generator(self, ply_data: np.array, grids_size: float, grids_step: int, margin: bool) -> Iterator[
    ...
    def exp_gen_bev_projection(self, grids_data: Iterator[Tuple[int, int, np.array]], ply_name) -> list:
        ...
        for grid in grids_data:
            ...
            alt, rgb, cla = hi.project_3d_bev_img3(pts.T, self.grids_scale, self.grids_step, self.label_color_map,
                                                   imgid)
