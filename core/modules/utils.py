...
def initial_voxelize(z, init_res, after_res):
    ...
    new_float_coord = torch.cat(
        [(z.C[:, :3] * init_res) / after_res, z.C[:, -1].view(-1, 1)], 1)  # 单独把坐标部分进行处理 反射率不处理直接复制

    pc_hash = F.sphash(torch.floor(new_float_coord).int())  # 取体素的下限 转成int 再生成每个idx的唯一哈希编码
    sparse_hash = torch.unique(pc_hash)  # 去除碰撞的hash？
    idx_query = F.sphashquery(pc_hash, sparse_hash)  # 找到对应的idx 映射成唯一的 这里貌似完全一致
    counts = F.spcount(idx_query.int(), len(sparse_hash))  # 计算每个voxel里面索引的数量？这里数量貌似都是1
